...
function verifyFathomSignature(webhookId, webhookTimestamp, webhookSignature, rawBody) {
  const secret = process.env.FATHOM_PERSONAL_WEBHOOK_SECRET || '';
  if (!secret) {
    console.warn('FATHOM_PERSONAL_WEBHOOK_SECRET not set — skipping verification');
    return true;
  }
...
async function fetchFathomMeetings() {
  const apiKey = process.env.FATHOM_PERSONAL_API_KEY;
  if (!apiKey) throw new Error('FATHOM_PERSONAL_API_KEY not set');
